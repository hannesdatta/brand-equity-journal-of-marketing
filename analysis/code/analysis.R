...
####################################
		...
		models[, descr := paste(index,endogeneity_spec,decay,model_type,sep='_')]
		...
		all_results <-clusterApplyLB(cl, model_descr, function(m) {
					...
					res=try(analyze_marketshares(dt, xvars_heterog=c('pi_bt', 'rreg_pr_bt', 'pct_store_skus_bt',paste0('adstock', adv_decay, '_bt')),
