...
library(MASS)
	...
	itersur <- function (X, Y, dates_brands, maxiter = 1000) {
		...
		# get maximum observations by brand, used for Kronecker computations
			obsperbrand = table(dates_brands$brand)
		...
		# resid index: goal per variable combination, I need to get residual index numbers with "matching" observations
		if(0){
		tmp = dcast(data.frame(dates_brands, index =1:nrow(dates_brands)), date ~ brand, value.var = "index")
			resid_index <- NULL
			cntr<-0
			for (.i in 1:length(obsperbrand)) {
				for (.j in 1:length(obsperbrand)) {
					cntr <- cntr+1
					resids = cbind(tmp[, .i + 1], tmp[, .j + 1])
					compl.cases = complete.cases(resids)
					if (length(which(compl.cases == T))<=1) {
						resid_index[[cntr]]<-integer(0) } else {
						resid_index[[cntr]]<-resids[complete.cases(resids),]
						}
					}
				}
		...
		#	backsolve(crossprod(X, omega_inverse) %*% X, diag(ncol(X))) #s
		
		# backsolve(sigma, diag(4), upper=TRUE)
		 
		 
#system.time(chol.sol <-


#+ backsolve(ch, forwardsolve(ch, crossprod(mm, y),  upper = TRUE, trans = TRUE)))
